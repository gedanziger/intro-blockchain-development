...
contract('Whitelist', async(accounts) => {
	...
	beforeEach(async function () {
  	...
  	describe('Add Whitelist', function () {
		it('should returns whitelisted result ', async function () {
			try {
				...
				const { logs } = await whitelistInstance.addWhitelist(signature, {from: accounts[1]});
				...
				// const { logs } = await whitelistInstance.checkWhitelist(signature);
			}
			catch (err) {
				console.log(err);
			}
